...
public class RobustMath {
    ...
    static class LogExpCache {
        ...
        static double lookupAdd(double val) {
            if (!useCache)
                return Math.log1p(Math.exp(-1*val));
            int index = 0;
    ...
    public static void logSumExp(DoubleMatrix1D v1, DoubleMatrix1D v2) {
...
//      System.out.println("Matrix "+M.size()+" "+M.columns()+ " "+logMult.cnt);
    ...
    public static DoubleMatrix1D logMult(DoubleMatrix2D M, DoubleMatrix1D y, DoubleMatrix1D z, double alpha, double beta, boolean transposeA, Object edgeGen) {
        // z = alpha * M * y + beta*z
        // in log domain this becomes: 
        ...
        for (int j = 0; j < M.columns(); j++) {
        	// Xuchen: possible bug fix. i shouldn't start from j when edgeGen is null
            // for (int i = (edgeGen==null?0:edgeGen.first(j)); i < M.rows(); i = (edgeGen==null)?i+1:edgeGen.next(j,i)) {
    ...
    public static DoubleMatrix1D Mult(DoubleMatrix2D M, DoubleMatrix1D y, DoubleMatrix1D z, double alpha, double beta, boolean transposeA, Object edgeGen) {
        // z = alpha * M * y + beta*z
