	...
	function writeHtmlAlignmentTable(sourceWords, targetWords, sureGrid, probGrid, 
		...
		for(i = 0; i < sourceWords.length; i++) {
			...
			html += '<div style="-webkit-transform: rotate(-30deg); -webkit-transform-origin: left bottom; \
				-moz-transform: rotate(-30deg); -moz-transform-origin: left bottom; z-index:inherit; float:center">';
			if(smallerFont) { 
...
//			html += '</td>\n\t';
		...
		for(i = 0; i < sourceWords.length; i++) {
			...
			html += '<div style="-webkit-transform: rotate(-30deg); -webkit-transform-origin: left top; \
				-moz-transform: rotate(-30deg); -moz-transform-origin: right bottom; z-index:inherit; float:center">';
			if(smallerFont) {
				html += '<font size=' + fontSize + '>';
			}
			html += word;
